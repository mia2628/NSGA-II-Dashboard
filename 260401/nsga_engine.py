...
AGING_ALPHA_HOURS = 0.35
AGING_GAMMA_ENV = 0.40
AGING_BETA_FR = 0.60
AGING_MULTIPLIER_CAP = 2.50
AGING_USE_EXP = True
...
def _build_engine_context(df_prepared: pd.DataFrame, config: NSGAConfig) -> Dict[str, object]:
    ...
    if len(positive_rad_hours) > 0:
        ...
        rad_hours_ref = 1.0
    hours_norm_all = np.clip(annual_rad_hours_all / max(rad_hours_ref, 1e-9), 0.0, 1.5)
    ...
    annual_fr_sp = FR_all[spare_idx] * aging_multiplier_all[spare_idx] * (1.0 - gamma_all[spare_idx])
...
def _simulate_population(
    ...
    managed_items = np.sum(Y > 0, axis=1).astype(int)
    total_stock_units = np.sum(total_stock, axis=1).astype(int)
    prebuy_selected = np.sum((Y > 0) & ctx["_prebuy_flag_bool"].reshape(1, -1), axis=1).astype(int)
    protection_selected = np.sum((total_stock > 0) & ctx["_protection_flag_bool"].reshape(1, -1), axis=1).astype(int)
    ...
    return {
        ...
        "managed_items": np.asarray(managed_items, dtype=int),
...
def _build_pareto_df(ctx: Dict[str, object], res_single: Dict[str, object], config: NSGAConfig) -> pd.DataFrame:
    ...
    df_pareto = pd.DataFrame({
        ...
        "managed_parts": np.asarray(sim_pop["managed_items"], dtype=int)[pareto_idx],
...
def run_nsga2(
    ...
    summary = {
        ...
        "best_managed_parts": int(best_sim["managed_items"]),
        ...
        "managed_parts": int(best_sim["managed_items"]),
