...
def build_explainability_tables(result: dict) -> dict:
    ...
    df["manage_flag"] = df.get("manage_flag", False).astype(bool)
    df["prebuy_flag"] = df.get("prebuy_flag", False).astype(bool)
    df["protection_flag"] = df.get("protection_flag", False).astype(bool)
    ...
    managed_compare = pd.DataFrame([
        {
            "group": "Managed",
            "impact_score": float(df.loc[df["manage_flag"], "impact_score"].mean()) if df["manage_flag"].any() else 0.0,
            "lead_time": float(df.loc[df["manage_flag"], "lead_time"].mean()) if df["manage_flag"].any() else 0.0,
            "priority_score": float(df.loc[df["manage_flag"], "priority_score"].mean()) if df["manage_flag"].any() else 0.0,
            "cost_burden": float(df.loc[df["manage_flag"], "cost_burden"].mean()) if df["manage_flag"].any() else 0.0,
            "xai_action_score": float(df.loc[df["manage_flag"], "xai_action_score"].mean()) if df["manage_flag"].any() else 0.0,
        },
        {
            "group": "Not Managed",
            "impact_score": float(df.loc[~df["manage_flag"], "impact_score"].mean()) if (~df["manage_flag"]).any() else 0.0,
    ...
    selected_count = int(df["manage_flag"].sum())
    total_count = int(len(df))
    solution_cards = {
        "selection_logic": f"전체 {total_count:,}개 품목 중 {selected_count:,}개 품목이 현재 대표 해에서 관리 대상으로 유지되었습니다.",
    ...
    why_solution = {
        ...
        "selection_bias": "선발주와 보호재고 규칙이 있는 품목은 설명 카드에서 별도 구분됩니다.",
        "risk_buffer": f"총 재고 {int(summary.get('total_stock_units', summary.get('total_stock', 0))):,}단위를 기준으로 다운타임 완충 전략을 해석합니다.",
        "tradeoff": "비용 부담 점수와 액션 우선순위 점수를 동시에 보며 의사결정 밸런스를 설명합니다.",
    }
    narrative = (
        f"대표 해 기준으로 관리 품목은 {selected_count:,}개이며, "
        f"주요 설명 축은 선발주·보호재고·Ao 영향도·리드타임·비용 대비 효과입니다."
    )
        ...
        if not dominated.empty:
            fig.add_trace(
                go.Scatter(
                    ...
                    legendgroup=name,
                    showlegend=True,
                    customdata=np.stack([dominated["target_ao"], dominated["solution_id"]], axis=1) if "target_ao" in dominated.columns else np.stack([np.zeros(len(dominated)), dominated["solution_id"]], axis=1),
                    ...
                    hovertemplate="Target Ao=%{customdata[0]:.2f}<br>Solution=%{customdata[1]}<br>Cost=%{x:,.2f}<br>Ao=%{y:.4f}<br>Pareto=N<extra></extra>",
                )
            )
        if not pareto.empty:
            fig.add_trace(
                go.Scatter(
                    ...
                    legendgroup=name,
                    showlegend=False,
                    customdata=np.stack([pareto["target_ao"], pareto["solution_id"]], axis=1) if "target_ao" in pareto.columns else np.stack([np.zeros(len(pareto)), pareto["solution_id"]], axis=1),
                    ...
                    hovertemplate="Target Ao=%{customdata[0]:.2f}<br>Solution=%{customdata[1]}<br>Cost=%{x:,.2f}<br>Ao=%{y:.4f}<br><b>Pareto=Y</b><extra></extra>",
                )
            )
...
def render_summary_panel(summary: dict):
    ...
    managed_parts = int(summary.get('best_managed_parts', summary.get('managed_parts', summary.get('managed_count', 0))))
    total_stock = int(summary.get('best_total_stock_units', summary.get('total_stock_units', summary.get('total_stock', 0))))
    ...
    selected_prebuy = int(summary.get('best_selected_prebuy', summary.get('selected_prebuy', 0)))
    selected_protection = int(summary.get('best_selected_protection', summary.get('selected_protection', 0)))
...
def render_xai(result: dict | None, df_input: pd.DataFrame | None):
    ...
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        show_metric_card("Selected", f"{int(xai.get('selected_count', 0)):,}")
    with c2:
        show_metric_card("Total", f"{int(xai.get('total_count', 0)):,}")
    with c3:
        show_metric_card("Reason Codes", f"{len(reason_counts):,}")
    with c4:
        show_metric_card("Decision Buckets", f"{len(bucket_counts):,}")
...
with st.sidebar:
    ...
    target_ao = st.slider("대표 Target Ao", min_value=0.10, max_value=0.99, value=0.94, step=0.01)
    pmin = st.slider("Pmin", min_value=0.00, max_value=1.00, value=0.10, step=0.01)
    long_lead_percentile = st.slider("장기 리드타임 분위", min_value=0.50, max_value=0.99, value=0.80, step=0.01)
    ao_impact_percentile = st.slider("Ao 영향도 분위", min_value=0.50, max_value=0.99, value=0.80, step=0.01)
    st.markdown(
        f"""
        ...
        <div class="small-note">실행 중 진행상황 갱신을 더 성기게 묶어서 처리하고, 엔진 내부 계산 재사용·벡터화·배치 offspring 생성·history 누적·정책 계산 재사용·2D Pareto 추출·요약 집계·mutation 난수 할당 최적화와 함께 sweep 및 generation 실행 속도를 높입니다.</div>
        """,
        unsafe_allow_html=True,
    )
