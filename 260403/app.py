...
def build_final_stock_relationship_table(stock_summary: dict) -> pd.DataFrame:
    ...
    rows = [
        {
            "구분": "Selected Pre-buy",
            "품목 수": _safe_int(stock_summary.get("selected_prebuy", 0)),
            "재고 단위": _safe_int(stock_summary.get("selected_prebuy_units", 0)),
            "설명": "선발주 규칙에 걸리면서 최종 권고 재고가 1개 이상인 품목",
        },
        {
            "구분": "Selected Protection",
            "품목 수": _safe_int(stock_summary.get("selected_protection", 0)),
            "재고 단위": _safe_int(stock_summary.get("selected_protection_units", 0)),
            "설명": "보호재고 규칙에 걸리면서 최종 권고 재고가 1개 이상인 품목",
        },
        {
            "구분": "Normal Stock",
            "품목 수": _safe_int(stock_summary.get("normal_stock_items", 0)),
            "재고 단위": _safe_int(stock_summary.get("normal_stock_units", 0)),
            "설명": "선발주/보호재고는 아니지만 최종적으로 재고 확보가 권고된 품목",
        },
        {
            "구분": "Managed Parts",
            "품목 수": _safe_int(stock_summary.get("managed_parts", 0)),
            "재고 단위": _safe_int(stock_summary.get("total_stock_units", 0)),
            "설명": "최종 재고가 실제 배정된 품목 수와 총 재고 단위",
        },
        {
            "구분": "Raw Managed Parts",
            "품목 수": _safe_int(stock_summary.get("raw_managed_parts", stock_summary.get("managed_parts", 0))),
            "재고 단위": 0,
            "설명": "Y>0 관리 의사결정 기준 품목 수(재고 배정 전 원시 관리 수)",
        },
        {
            "구분": "산술 검증",
            "품목 수": _safe_int(stock_summary.get("stock_category_parts_sum", 0)),
            "재고 단위": _safe_int(stock_summary.get("stock_category_units_sum", 0)),
            "설명": "Selected Pre-buy + Selected Protection + Normal Stock의 합",
        },
    ]
    return pd.DataFrame(rows)
...
def render_summary_panel(summary: dict, policy_df: pd.DataFrame | None = None):
    ...
    managed_parts = _safe_int(stock_summary.get('managed_parts', summary.get('best_managed_parts', summary.get('managed_parts', summary.get('managed_count', 0)))))
    raw_managed_parts = _safe_int(stock_summary.get('raw_managed_parts', summary.get('best_raw_managed_parts', managed_parts)))
    total_stock = _safe_int(stock_summary.get('total_stock_units', summary.get('best_total_stock_units', summary.get('total_stock_units', summary.get('total_stock', 0)))))
    total_cost = _safe_float(summary.get('best_cost', summary.get('total_cost', 0.0)))
    dt_wait = _safe_float(summary.get('best_DT_wait_h', summary.get('DT_wait_h', 0.0)))
    selected_prebuy = _safe_int(stock_summary.get('selected_prebuy', summary.get('best_selected_prebuy', summary.get('selected_prebuy', 0))))
    selected_protection = _safe_int(stock_summary.get('selected_protection', summary.get('best_selected_protection', summary.get('selected_protection', 0))))
    normal_stock_items = _safe_int(stock_summary.get('normal_stock_items', summary.get('best_normal_stock_items', summary.get('normal_stock_items', 0))))
    prebuy_units = _safe_int(stock_summary.get('selected_prebuy_units', summary.get('best_selected_prebuy_units', 0)))
    protection_units = _safe_int(stock_summary.get('selected_protection_units', summary.get('best_selected_protection_units', 0)))
    normal_units = _safe_int(stock_summary.get('normal_stock_units', summary.get('best_normal_stock_units', 0)))
    ...
    st.markdown(
        ...
        <div class="section-box">
            ...
            <div style="line-height:1.8;color:#3d5f45;">
                ...
                이 해의 최종 재고 구성은 <b>Managed Parts {managed_parts:,}개</b>, <b>Total Stock Units {total_stock:,}단위</b>,
                <b>Selected Pre-buy {selected_prebuy:,}개</b>, <b>Selected Protection {selected_protection:,}개</b>, <b>Normal Stock {normal_stock_items:,}개</b>입니다.
                즉, 목표 가용도를 만족시키기 위해 어떤 수리부속을 얼마나 확보해야 하는지와 그 구성을 함께 보여주는 최종 의사결정 결과입니다.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    with c3:
        show_metric_card("Managed Parts", f"{managed_parts:,}")
    with c4:
        show_metric_card("Total Stock Units", f"{total_stock:,}")
    with c5:
        show_metric_card("Normal Stock", f"{normal_stock_items:,}")
    with c6:
        show_metric_card("Total Cost", f"{total_cost:,.0f}")

    detail_rows = [
        {"항목": "Managed Parts", "값": f"{managed_parts:,}", "의미": "최종적으로 재고가 1개 이상 배정된 품목 수"},
        {"항목": "Raw Managed Parts", "값": f"{raw_managed_parts:,}", "의미": "Y>0 관리 의사결정 기준 품목 수"},
        {"항목": "Selected Pre-buy", "값": f"{selected_prebuy:,}개 / {prebuy_units:,}단위", "의미": "조기 확보가 필요한 핵심 선발주 품목 수와 배정 단위"},
        {"항목": "Selected Protection", "값": f"{selected_protection:,}개 / {protection_units:,}단위", "의미": "다운타임 완충을 위해 보호재고가 필요한 품목 수와 배정 단위"},
        {"항목": "Normal Stock", "값": f"{normal_stock_items:,}개 / {normal_units:,}단위", "의미": "선발주/보호재고 외 일반 재고 확보 품목 수와 배정 단위"},
        {"항목": "Total Stock Units 산식", "값": f"{prebuy_units:,} + {protection_units:,} + {normal_units:,} = {total_stock:,}", "의미": "Total Stock Units = Selected Pre-buy + Selected Protection + Normal Stock"},
        {"항목": "DT_wait_h", "값": f"{dt_wait:,.2f}", "의미": "부족으로 인한 대기 다운타임 수준"},
    ]
    st.dataframe(pd.DataFrame(detail_rows), use_container_width=True, hide_index=True, height=285)
...
def render_xai(result: dict | None, df_input: pd.DataFrame | None):
    ...
    with c1:
        show_metric_card("Total Unit Stock", f"{_safe_int(stock_summary.get('total_stock_units', 0)):,}")
    with c2:
        show_metric_card("Managed Parts", f"{_safe_int(stock_summary.get('managed_parts', xai.get('selected_count', 0))):,}")
    with c3:
        show_metric_card("Selected Pre-buy", f"{_safe_int(stock_summary.get('selected_prebuy', 0)):,}")
    with c4:
        show_metric_card("Selected Protection", f"{_safe_int(stock_summary.get('selected_protection', 0)):,}")
    with c5:
        show_metric_card("Normal Stock", f"{_safe_int(stock_summary.get('normal_stock_items', 0)):,}")
    ...
    with rel_right:
        st.markdown(
            ...
            <div class='section-box'>
                ...
                2) Selected Pre-buy / Selected Protection / Normal Stock도 재고가 실제로 배정된 품목만 셉니다.<br><br>
                3) 따라서 최종 결과는 <b>Total Unit Stock = Selected Pre-buy + Selected Protection + Normal Stock</b> 구조로 해석할 수 있습니다.<br><br>
                4) 이 화면의 숫자는 policy_df에 저장된 대표 해의 NSGA 결과를 다시 집계한 값입니다.
            </div>
            """,
            unsafe_allow_html=True,
        )
...
st.markdown(
    ...
    <div class="hero-box">
        ...
        <div class="hero-sub" style="margin-top:8px;opacity:0.96;">이번 버전은 failure_rate 원값을 사용하며 aging 보정은 적용하지 않습니다.</div>
    </div>
    """,
    unsafe_allow_html=True,
)
